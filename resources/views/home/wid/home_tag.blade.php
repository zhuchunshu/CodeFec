<div class="col-md-12">
    <div class="card">
        <div class="card-status-top bg-{{ $tags['color'] }}"></div>
        <div class="card-body">
            <h3 class="card-title badge bg-{{ $tags['color'] }}" style="min-height: 35px">
                {{ $tags['name'] }}
            </h3>
        </div>
        <div class="card-footer">
            <form onsubmit="return confirm('您确定要删除此标签吗?');" action="{{ route('admin.node.tags.delete') }}"
                method="POST">
                @if (Auth::id())
                    <a href="{{ route('create.topic') }}" class="btn btn-{{ $tags['color'] }}">
                        发帖
                    </a>
                @else
                    <span>
                ...
                @if (CodeFec_Quanxian()->_(777))
                    @csrf
                    <input type="hidden" name="ename" value="{{ $tags['ename'] }}">
                    <button type="submit" class="btn btn-{{ $tags['color'] }}">
                        删除此标签
                    </button>
                @endif
            </form>
