...
class TopicController extends Controller
{
    ...
    public function create_post_draft(array $data)
    {
        $id = Topic::insertGetId([
            ...
            'ishtml' => 'yes',
            'tlou_shuliang' => $data['tlou_jiangli'],
            'tlou_date' => $data['tlou_jieshu'],
            'created_at' => date("Y-m-d H:i:s"),
            'updated_at' => date("Y-m-d H:i:s"),
            'token' => Str::random(15)
        ]);
        if ($id) {
            topic_options('options_reply',$id,$data['options_reply']);
            topic_options('options_whosee',$id,$data['options_whosee']);
            topic_options('options_quanxian',$id,$data['options_quanxian']);
    ...
    public function create_post_publish(array $data)
    {
        if ($data['tlou_on'] == "on") {
            // 开启T楼
            if ($data['tlou'] && $data['tlou_jiangli'] && $data['tlou_jieshu']) {
                return $this->create_post_publish_insert((array)$data);
            } else {
                session()->flash('danger', '您已开启T楼功能,请把T楼信息配置完整');
                return redirect()->back();
            }
        } else {
            // 不开启T楼
    ...
    private function create_post_publish_insert(array $data)
    {
        ...
        // 发布
        if($data['tlou'] && Tlou_time($data['tlou_jieshu'])){
            session()->flash('danger','T楼结束时间必须大于当前时间');
            return redirect()->back();
        }
        $id = Topic::insertGetId([
            'title' => $data['title'],
    ...
    public function draft_post(DarftRequest $request, Topic $topic)
    {
        ...
        if (@$data['caogao'] == 'yes') {
            ...
            topic_options('options_reply',$data['id'],$data['options_reply']);
            topic_options('options_whosee',$data['id'],$data['options_whosee']);
            topic_options('options_quanxian',$data['id'],$data['options_quanxian']);
    ...
    public function show($id,Topic $topic,PostsLike $pl,PostsComment $postsComment,Report $report,Request $request){
        if($topic->where([['id',$id],['publish','发布']])->count()){
            ...
            if($data['tlou']){
                return $this->tlou_show($data,$like,$comment,$get_topic,$comment_count,$ilike,$huojiang);
            }else{
    ...
    // T楼帖子
    public function tlou_show($data,$like,$comment,$get_topic,$comment_count,$ilike,$huojiang){

        if(Tlou_time($data['tlou_date'])){
            if($comment_count>= $data['tlou_shuliang']){
                // 颁奖
                if(TlouModels::where(['topic_id' => $data['id']])->count() < $data['tlou_shuliang']){
                    // 随机发奖
                    $comment_data = PostsComment::where(['topic_id' => $data['id']])->inRandomOrder()->take((int)$data['tlou_shuliang'])->get();
                    foreach ($comment_data as $value) {
                        if(!TlouModels::where(['topic_id' => $data['id'],'user_id' => $value['user_id']])->count()){
                            TlouModels::insert([
                                'topic_id' => $data['id'],
                    ...
                    // 获奖人
                    $huojiang = TlouModels::where(['topic_id' => $data['id']])->get();
                }else{
                    $huojiang = TlouModels::where(['topic_id' => $data['id']])->get();
                }
            }
        }else{
            if($comment_count>= $data['tlou']){
                if($comment_count>= $data['tlou_shuliang']){
                    // 颁奖
                    if(TlouModels::where(['topic_id' => $data['id']])->count() < $data['tlou_shuliang']){
                        // 随机发奖
                        ...
                        // 获奖人
                        $huojiang = TlouModels::where(['topic_id' => $data['id']])->with('user')->get();
                    }else{
                        $huojiang = TlouModels::where(['topic_id' => $data['id']])->with('user')->get();
                    }
                }
    ...
    public function edit_save(EditSave $request,Topic $topic){
        ...
        if ($data['tlou_on'] == "on") {
            ...
            // 不开启T楼
            $data['tlou']=null;
            $data['tlou_jiangli']=null;
            $data['tlou_jieshu']=null;
            return $this->edit_save_post_publish((array)$data);
        }
    }
